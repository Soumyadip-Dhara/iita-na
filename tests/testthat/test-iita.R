test_that("iita_na works with complete data", {
...
test_that("iita_na works with missing data", {
...
test_that("iita_na handles all missing data", {
...
test_that("iita_na validates input", {
  # Non-binary data should throw error
  data <- matrix(c(0, 1, 2, 3), ncol = 2)
  expect_error(iita_na(data), "must contain only 0, 1, or NA")
  
  # Invalid selrule
  data <- matrix(c(0, 1, 0, 1), ncol = 2)
  expect_error(iita_na(data, selrule = "invalid"), "must be either 'minimal' or 'corrected'")
...
test_that("transitive_closure works correctly", {
  ...
  # Access internal function - note: transitive_closure is not exported
  # but it's called internally by generate_quasiorders
...
test_that("iita_na produces consistent results for same data", {
  data <- matrix(c(
    0, 0, 0,
    1, 0, 0,
    1, 1, 0,
...
test_that("print.iita_na works without error", {
...
test_that("iita_na handles single item", {
...
test_that("iita_na corrected selection rule works", {
  data <- matrix(c(
    0, 0, 0,
    1, 0, 0,
    1, 1, 0,
